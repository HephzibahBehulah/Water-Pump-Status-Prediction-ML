"""
SMOTE + Random Forest Training - Handle Class Imbalance

Applies SMOTE to balance the training data, then trains Random Forest
This helps the model learn the minority class better.

Run: python scripts/train_smote_randomforest.py
"""
...
logger = setup_logger(__name__, log_level="INFO", log_file="logs/training_smote_rf.log")
...
def main():
    logger.info("="*60)
    logger.info("SMOTE + Random Forest - Class Imbalance Handling")
    logger.info("="*60)
    ...
    try:
        ...
        logger.info("\n[Random Forest] Training Random Forest with SMOTE-balanced data...")
        ...
        with mlflow.start_run(run_name="randomforest_smote_balanced"):
            ...
            joblib.dump(best_rf, models_dir / "randomforest_smote.pkl")

            results = {
                "model": "randomforest_with_smote",
            ...
            with open("metrics_smote_randomforest.json", "w") as f:
                json.dump(results, f, indent=2)

        logger.info("\n" + "="*60)
        logger.info("TRAINING COMPLETED!")
        logger.info("="*60)
        logger.info(f"Results: metrics_smote_randomforest.json")
        logger.info(f"Model: models/production/randomforest_smote.pkl")
